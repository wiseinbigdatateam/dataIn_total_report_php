...
if(in_array($mode, array("report_insert", "report_temporary", "report_analyzing"))) {
    ...
    //요소만족도 있으면 해당 variable_element 값 1, 같은 모델 타입 variable_element 값 2
    //요소만족도 없으면 0
    print_r($satis_model_quizno);
    ...
    if(is_array($variable_element)){
        foreach ($variable_element as $model_type => $variable_element_list) {
            if($variable_element_list != '') {
                $sql = "UPDATE MM_analysis_report_model
