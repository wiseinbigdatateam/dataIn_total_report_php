...
<?
	...
	$sql_file_1 = "select * from wise_analysis_report_service_result_totalrate where 1 and service_option_idx='".$service_option_idx."' order by idx asc"; 
	$query_file_1 = mysqli_query($gconnet,$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1); // 점수변환(100점예시)_총점&등급산출
?>
<body>
    <header>
        <div class="logo">
    ...
    <section>
        <div class="body_w">
		 <form name="frm" action="myreport_save_ing.php" target="_fra_admin" method="post"  enctype="multipart/form-data">
			...
			<input type="hidden" name="report_type" id="report_type" value="service"/>
			<input type="hidden" id="report_idx" name="report_idx" value="<?=$service_option_idx?>"> 
			<input type="hidden" id="report_name" name="report_name" value="<?=$row['analysis_report_service_option_name']?>"> 
                    ...
                    <div class="mf_wrap">
                         <div class="form_wrap">
								<div class="form">
                                    ...
                                    <span class="jo_name"><?=$row['analysis_report_service_option_name']?></span>
                                </div>
                                <div class="form">
                                    <label for="name" class="label_name">모집단</label>
                                    <span class="peo_name"><?=$row['analysis_report_service_option_population']?></span>
                                </div>
                                <div class="form">
                                    <label for="name" class="label_name">표본크기</label>
                                    <span class="cus_name"><?=$row['analysis_report_service_option_sampling']?></span>
                                </div>
                                <div class="form">
                                    <label for="name" class="label_name">표본추출방법</label>
                                    <span class="way_name"><?=$row['analysis_report_service_option_samplingmethod']?></span>
                                </div>
                                <div class="form">
                                    <label for="name" class="label_name">조사방법</label>
                                    
                                    <span class="sur_name"><?=$row['analysis_report_service_option_surveymethod']?></span>
                                </div>
                                 <div class="form">
                                    <label for="name" class="label_name">조사기간</label>
                                    <span class="date_name"><?=$row['analysis_report_service_option_data']?></span>
                                </div>
                                <!-- <div class="m5_logo">
                                    <img src="<?=$_P_DIR_WEB_FILE?>service/<?=$row['analysis_report_service_option_logo_c']?>" style="max-width:80%;">
                                </div> -->
                                ...
                                <div class="save_info">
                                    <p>현재 설정을 저장하시겠습니까?</p>
                                    <p>파일명 지정</p>
                                    <input type="text" placeholder="공원만족도조사_만족도모델보고서_20200920" id="service_option_report_title" name="service_option_report_title" required="yes" message="보고서 파일명">
                                    <div class="save_btn">
            ...
            <div class="btn_all last">
                <div class="left_btn">
                    <button type="button" onclick="location.href='service_3.php?service_option_idx=<?=$service_option_idx?>';">이전 단계</button>
                </div>
