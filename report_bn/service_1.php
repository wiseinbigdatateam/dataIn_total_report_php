...
<?
	...
	$sql_file_1 = "select * from wise_analysis_report_service_option_group where 1 and service_option_idx='".$service_option_idx."' order by idx asc"; // 집단 구분 항목 
...
<body>
    ...
    <section>
        <div class="body_w s1">
        <form name="frm" action="service_1_action.php" target="_fra_admin" method="post"  enctype="multipart/form-data">
			...
			<input type="hidden" id="analysis_report_option_idx" name="analysis_report_option_idx" value="2"> <!-- 서비스 평가모델 보고서 -->
			<input type="hidden" id="analysis_report_option_title" name="analysis_report_option_title" value="서비스 평가모델 보고서">
			<input type="hidden" name="attach_count_1" id="attach_count_1" value="<?=$query_file_1_cnt?>"/> <!-- 집단 구분 항목 -->
			<input type="hidden" id="factor_no_x_p" name="factor_no_x_p"> <!-- 집단 구분 항목 -->    
            ...
            <div class="main_pro">
                ...
                <div class="guide_c">
                    ...
                    <div class="cover_gc tab7">
                        ...
                        <div class="gc_wrap">
                            <span class="red_exc">!</span>
                            <span>" - " 버튼을  클릭시 집단구분항목이 삭제됩니다.</span>
                        </div>
                        <div class="gc_wrap">
                            <span class="red_exc">!</span>
                            <span>하단의 " + " 버튼을 클릭시 항목 입력란이 추가됩니다.</span>
                        </div>
                    </div>
                ...
                <div class="main_flow main_ser1 content-rd">
                    <div class="mf_wrap">
                         <div class="form_wrap">
                                <div class="p1_wrap tabb1">
                                    <span class="title_1p">조사명</span>
                                    <input type="text" class="input" placeholder="20XX년 전화친절조사" id="service_option_name" name="service_option_name" value="<?=$row['analysis_report_service_option_name']?>" required="no" message="조사명">
                                </div>
                                <div class="p1_wrap tabb2">
                                    <span class="title_1p">모집단</span>
                                    <input type="text" class="input" placeholder="도로교통공단 소속 46개 지부/교통방송/운전면허시험장에서 근무하는 직원" id="service_option_population" name="service_option_population" value="<?=$row['analysis_report_service_option_population']?>" required="no" message="모집단">
                                </div>
                                <div class="p1_wrap tabb3">
                                    <span class="title_1p ">표본크기</span>
                                    <input type="text" class="input" placeholder="705 Call (지부 : 216Call, 지방교통방송: 105Call, 운전면허시험장: 384Call)" id="service_option_sampling" name="service_option_sampling" value="<?=$row['analysis_report_service_option_sampling']?>" required="no" message="표본크기">
                                </div>
                                <div class="p1_wrap tabb4">
                                    <span class="title_1p">표본추출방법</span>
                                    <input type="text" class="input" placeholder="각 부서별 인원 수를 고려한 유의할당 추출(Purposive Sampling)" id="service_option_samplingmethod" name="service_option_samplingmethod" value="<?=$row['analysis_report_service_option_samplingmethod']?>" required="no" message="표본추출방법">
                                </div>
                                <div class="p1_wrap tabb5">
                                    <span class="title_1p">조사방법</span>
                                    <input type="text" class="input" placeholder="고객을 가장한 전화 모니터링(Mystery shopping)" id="service_option_surveymethod" name="service_option_surveymethod" value="<?=$row['analysis_report_service_option_surveymethod']?>" required="no" message="조사방법">
                                </div>
                                 <div class="p1_wrap tabb6">
                                    <span class="title_1p">조사기간</span>
                                    <input type="text" class="input " placeholder="20XX년 XX월 XX일 ~ 20XX년 XX월 XX일" id="service_option_data" name="service_option_data" value="<?=$row['analysis_report_service_option_data']?>" required="no" message="조사기간">
                                </div>
							<?
                                    ...
                                    <div class="ser_wrap ser">
                                        ...
                                        <div class="icon_box ">
                                            <div class="ser_btn" id="select_type_x_p<?=($opt_i+1)?>">
                                                <div class="ser_add">
                                                    <button class="ser_bbd bbd-base" type="button"><img src="./img/plus_sq.png" alt=""></button>
                                                </div>
												<?if($row_file_1['analysis_report_service_option_group_no']){?>
													<li id="ql_<?=$row_file_1['analysis_report_service_option_group_no']?>" class="ui-widget-content item_serv ui-selectee ui-selected selected-flag"><span class="close ui-selectee" style=""><button class="ui-selectee"><img src="./img/remove.png" alt="" class="ui-selectee"></button></span><a href="javascript:<?=$row_file_1['analysis_report_service_option_group_no']?>;" class="ui-selectee"><?=$row_file_1['analysis_report_service_option_group_title']?></a></li>
												<?}?>
                                            </div>
                                        </div>
							...
							<?}?>
								...
								<input type="hidden" name="dfile_old_name" id="dfile_old_name" value="<?=$row['analysis_report_service_option_logo_c']?>"/>
								<input type="hidden" name="dfile_old_org" id="dfile_old_org" value="<?=$row['analysis_report_service_option_logo_o']?>"/>
								...
								<?if($row['analysis_report_service_option_logo_o']){?>	
								 <div class="delete_check">
                                    <span class="dc-name">현재파일 : <a href="/report/pro_inc/download_file.php?nm=<?=$row['analysis_report_service_option_logo_c']?>&on=<?=$row['analysis_report_service_option_logo_o']?>&dir=service"><?=$row['analysis_report_service_option_logo_o']?></a></span>
                                    <label for="del-check">
...
<script>
	...
	function go_submit() {
		...
		if(check) {
			...
			//var attach_count_1 = "<?=$query_file_1_cnt?>"; 
			var str_type_x_p = '';
