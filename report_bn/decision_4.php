...
<body>
    ...
    <section>
		...
		<form name="frm" action="myreport_save_ing.php" target="_fra_admin" method="post"  enctype="multipart/form-data">
		 <!--<form name="frm" action="decision_4_action.php" target="_fra_admin" method="post"  enctype="multipart/form-data">-->
			...
			<input type="hidden" name="report_type" id="report_type" value="decision"/>
			<input type="hidden" id="report_idx" name="report_idx" value="<?=$decision_option_idx?>"> 
			<input type="hidden" id="report_name" name="report_name" value="<?=$row['analysis_report_decision_option_population']?>"> 
           ...
           <div class="side_men">
            ...
            <div class="main_pro">
			   ...
			   <div class="m_paging">
                ...
                <div class="dec_save content-rd">
                    <div class="mf_wrap">
                           <div class="form_wrap">
								<div class="form">
                                    <label for="name" class="label_name">모집단</label>
                                    <span class="peo_name"><?=$row['analysis_report_decision_option_population']?></span>
                                </div>
                                <div class="form">
                                    <label for="name" class="label_name">조사대상</label>
                                    <span class="cus_name"><?=$row['analysis_report_decision_option_sampling']?></span>
                                </div>
                                <div class="form">
                                    <label for="name" class="label_name">표본추출방법</label>
                                    <span class="way_name"><?=$row['analysis_report_decision_option_samplingmethod']?></span>
                                </div>
                                <div class="form">
                                    <label for="name" class="label_name">조사방법</label>
                                    <span class="sur_name"><?=$row['analysis_report_decision_option_surveymethod']?></span>
                                </div>
                                 <div class="form">
                                    <label for="name" class="label_name">조사기간</label>
                                    <span class="date_name"><?=$row['analysis_report_decision_option_data']?></span>
                                </div>
                                <!-- <div class="m5_logo">
                                     <img src="<?=$_P_DIR_WEB_FILE?>decision/<?=$row['analysis_report_decision_option_logo_c']?>" style="max-width:80%;">
                                </div> -->
                                ...
                                <div class="save_info">
                                    <p>현재 설정을 저장하시겠습니까?</p>
                                    <p>파일명 지정</p>
                                    <input type="text" placeholder="<?=$row['analysis_report_decision_option_logo_o']?>" id="decision_option_report_title" name="decision_option_report_title" required="yes" message="보고서 파일명">
                                    <div class="save_btn">
			...
			<div class="btn_all last">
                <div class="left_btn" onclick="location.href='decision_3.php?decision_option_idx=<?=$decision_option_idx?>';">
                    <button>이전 단계</button>
                </div>
