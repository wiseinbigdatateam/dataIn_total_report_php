...
*/
		...
		if(!$i_height){
			...
			$w_rate = zero_point_set(($w/$i_width),2); 
			$i_height = zero_point_set(($h/$w_rate),0); 
...
function img_resize( $srcimg, $dstimg, $imgpath, $rewidth, $reheight,$watermark_sect ) {
	...
	if($rewidth < $src_info[0] || $reheight < $src_info[1]) {
		if(($src_info[0] - $rewidth) > ($src_info[1] - $reheight)) {
			$reheight = zero_point_set(($src_info[1]*$rewidth)/$src_info[0]);
		} else {
			$rewidth = zero_point_set(($src_info[0]*$reheight)/$src_info[1]);
		}
	} else {
...
function get_minustime($strtime) { 
	...
	$min = zero_point_set($calculate_time/60)."분"; //분
    if($min >60){
		 $min = zero_point_set($calculate_time/3600)."시간"; //시간
		 if($min > 24){
			$min = zero_point_set($calculate_time/86400)."일"; //일
		}
   }
   return $min; 
} 
...
function get_minustime_req($reqtime,$strtime) { 
	if(strtotime($reqtime) <= strtotime($strtime)){
		...
		$min = zero_point_set($calculate_time/60)."분 후"; //분
		if($min >60){
			 $min = zero_point_set($calculate_time/3600)."시간 후"; //시간
			if($min > 24){
				$min = zero_point_set($calculate_time/86400)."일 후"; //일
			}
		}
	}
...
function get_distance($lat1, $lon1, $lat2, $lon2) {
  ...
  return zero_point_set($b*$A*($sigma-$deltaSigma) / 1000,1);
...
function get_star_avg($bbs_code,$target_idx,$bbs_sect){
	...
	if($num_sub == 0){
		...
		$avg_star = zero_point_set($row_star_sub[after_point]/$num_sub,1);	
	}
...
function get_star_per_part($bbs_code,$target_idx,$tpart,$tpartcon,$bbs_sect){
	...
	if($num_cnt == 0){
		...
		$avg_star = zero_point_set($num_sub/$num_cnt,1);	
	}
...
function get_calcurate_point($satisfaction_option_idx,$orgscore){
	...
	$calcurate_point = $calcurate_point;

	//echo "zeroed calcurate_point = ".$calcurate_point."<br>";
...
function get_calcurate_point_service($service_option_idx,$orgscore){
	...
	//$calcurate_point = zero_point_set($calcurate_point,$service_option_scorepoint);
	$calcurate_point = $calcurate_point;
	return $calcurate_point;
}
...
function get_satisf_option_model_quiz_avg($satisfaction_option_100data_idx,$data_no,$satisfaction_option_idx,$option_model_idx){
	...
	if($opt_cnt == 0){
		...
		$avg_point = $total_point/$opt_cnt;
		//echo "평균값 = ".$avg_point."<br>";
	}
	
	//echo $option_model_idx." / total_point = ".$total_point."<br>";
	//echo $option_model_idx." / total_cnt = ".$opt_cnt."<br>";

	//echo "중분류값 = ".$avg_point."<br>";
	//$avg_point = zero_point_set($avg_point,$satisfaction_option_scorepoint);
	$avg_point = $avg_point;
	return $avg_point;
...
function get_satisf_csi_avg($satisfaction_option_idx,$satisfaction_option_100data_idx,$data_no){
	...
	$sql_file_1 = "select * from wise_analysis_report_satisfaction_option_model where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_option_variable_type in ('X','M') order by idx asc"; // 독립 및 중간변수 
	...
	$avg_cnt_1 = $query_file_1_cnt;

	$csi_data_tot_1 = 0;
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ // 독립 및 중간변수 루프 시작 
		$row_file_1 = mysqli_fetch_array($query_file_1);
			
		$sql_file_2 = "select idx,(select quiz_no from wise_analysis_quiz where 1 and idx=a.analysis_report_satisfaction_option_factor_no) as quiz_no from wise_analysis_report_satisfaction_option_model_quiz a where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and satisfaction_option_model_idx='".$row_file_1['idx']."' and analysis_report_satisfaction_option_factor_type='P' order by idx asc"; // 자체변수만 
		...
		//echo "query_file_2_cnt = ".$query_file_2_cnt."<br>";

		$csi_data_1 = 0;
		for($opt_i2=0; $opt_i2<$query_file_2_cnt; $opt_i2++){ // 문항루프 시작 
		...
		$csi_data_2 = $csi_data_1/$query_file_2_cnt; // 문항변수 합계 평균 
		//echo $row_file_1['idx']." 박스점수 평균  = ".$csi_data_2."<br>";
		if($row_file_1['analysis_report_satisfaction_option_variable_weight']){
			$weight_yn = "Y";
			$csi_data_2 = ($csi_data_2/100) * $row_file_1['analysis_report_satisfaction_option_variable_weight']; // 평균에 가중치 퍼센트 곱 
		}
		//echo "문항변수 합계평균 csi_data_2 = ".$csi_data_2."<br>";
		$csi_data_tot_1 = $csi_data_tot_1+$csi_data_2;
	} // 독립 및 중간변수 루프 종료  

	//echo " 박스점수 총 합계  = ".$csi_data_tot_1."<br>";

	$sql_file_1 = "select * from wise_analysis_report_satisfaction_option_model where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_option_variable_type in ('X') and idx in (select satisfaction_option_model_idx from wise_analysis_report_satisfaction_option_model_quiz where 1 and analysis_report_satisfaction_option_factor_type='C') order by idx asc"; // 독립변수 
	//echo "sql_file_1 = ".$sql_file_1."<br>";
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	$avg_cnt_2 = $query_file_1_cnt;
	...
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ // 독립변수 루프 시작 
		...
		if($query_file_2_cnt == 0){
			...
			$csi_data_1 = 0;
			for($opt_i2=0; $opt_i2<$query_file_2_cnt; $opt_i2++){ // 요소변수별 문항루프 시작 
				$row_file_2 = mysqli_fetch_array($query_file_2);
			...
			$csi_data_2 = $csi_data_1/$query_file_2_cnt; // 요소변수 합계 평균 
			//echo " 요소점수 평균 = ".$csi_data_2."<br>";
			if($row_file_1['analysis_report_satisfaction_option_variable_elementweight']){
				$weight_yn = "Y";
				$csi_data_2 = ($csi_data_2/100) * $row_file_1['analysis_report_satisfaction_option_variable_elementweight']; // 평균에 요소 가중치 퍼센트 곱 
			}
			//echo "독립변수 가중치 csi_data_2 = ".$csi_data_2."<br>";
			$csi_data_tot_2 = $csi_data_tot_2+$csi_data_2;
		}
	} // 독립변수 루프 종료  
	
	//echo " 요소점수 총 합계  = ".$csi_data_tot_2."<br>";
	$avg_cnt_tot = $avg_cnt_1+$avg_cnt_2;

	$csi_data_tot = $csi_data_tot_1+$csi_data_tot_2;
	if($weight_yn == "N"){
		$csi_data_tot = $csi_data_tot/$avg_cnt_tot;
	}

	//$csi_data_tot = zero_point_set($csi_data_tot,$satisfaction_option_scorepoint);
	$csi_data_tot = $csi_data_tot;
	//echo "csi_data_tot = ".$csi_data_tot."<br><br>";
	return $csi_data_tot;
}
...
function get_satisf_data_group_detail_avg($satisfaction_option_idx,$analysis_report_idx,$quiz_no,$quiz_val,$quiz_no_tg){
	...
	$opt_sql = "select analysis_report_satisfaction_100data_no,(select quiz_title from wise_analysis_quiz where 1 and analysis_idx='".$analysis_report_idx."' and quiz_no='".$quiz_no."') as quiz_title from wise_analysis_report_satisfaction_100data_detail where satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_quiz_no='".$quiz_no."' and analysis_report_satisfaction_100data_quiz_val = '".$quiz_val."' and analysis_report_satisfaction_100data_no in (select analysis_report_satisfaction_100data_no from wise_analysis_report_satisfaction_100data_detail where 1 and analysis_report_satisfaction_100data_quiz_no in (select quiz_no from wise_analysis_quiz where 1 and analysis_idx='".$analysis_report_idx."'))";

	//echo "get_satisf_data_group_detail_avg_sql1 = ".$opt_sql."<br>"; //exit;
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
	
	//echo "get_satisf_data_group_detail_avg_opt_cnt = ".$opt_cnt."<br>"; //exit;
	
	if($opt_cnt == 0){
		$avg_point = zero_point_set("0",$satisfaction_option_scorepoint);
	} else {
		$total_point = 0;
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){ // 각 변수별 문항루프 시작 
			$opt_row = mysqli_fetch_array($opt_query);
			...
			$sql_data_2 = "select analysis_report_satisfaction_100data_quiz_val,(select quiz_title from wise_analysis_quiz where 1 and analysis_idx='".$analysis_report_idx."' and quiz_no='".$quiz_no_tg."') as quiz_title from wise_analysis_report_satisfaction_100data_detail where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_no='".$data_no."' and analysis_report_satisfaction_100data_quiz_no='".$quiz_no_tg."'";
		
			//echo "get_satisf_data_group_detail_avg_sql1 = ".$sql_data_2."<br>"; //exit;
			$query_data_2 = mysqli_query($GLOBALS['gconnet'],$sql_data_2);
			$row_data_2 = mysqli_fetch_array($query_data_2);
			//echo "개별 데이터 = ".$row_data_2['analysis_report_satisfaction_100data_quiz_val']."<br>";
			$total_point = $total_point + $row_data_2['analysis_report_satisfaction_100data_quiz_val'];
		...
		//echo "합계 데이터 = ".$total_point."<br>";
		//exit;
		$avg_point = zero_point_set($total_point/$opt_cnt,$satisfaction_option_scorepoint);
	}
...
function get_satisf_data_group_detail_tot($satisfaction_option_idx,$analysis_report_idx,$quiz_no,$quiz_val,$quiz_no_tg){
...
function get_satisf_data_group_detail_avg2($satisfaction_option_idx,$analysis_report_idx,$quiz_no,$quiz_val,$quiz_model="",$quiz_title=""){
	...
	//echo "get_satisf_data_group_detail_avg2 opt_sql = ".$opt_sql."<br>"; //exit;
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
	//echo "get_satisf_data_group_detail_avg2 opt_cnt = ".$opt_cnt."<br>";
	...
	if($opt_cnt == 0){
		...
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){ // 각 변수별 문항루프 시작 
			...
			$sql_data_2 = "select analysis_report_satisfaction_100data_quiz_val from wise_analysis_report_satisfaction_100data_detail where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_no='".$data_no."' and analysis_report_satisfaction_100data_model_no='".$quiz_model."'"; 
		...
		$avg_point = zero_point_set($total_point/$opt_cnt,$satisfaction_option_scorepoint);
		/*if($avg_point == "3.187"){
			echo "totp = ".$total_point."<br>";
			echo "totcnt = ".$opt_cnt."<br>";
			echo "totp / totcnt = ".($total_point/$opt_cnt)."<br>";
		}*/
	}
	//exit;
	return $avg_point;
...
function get_satisfaction_100data_no_val($satisfaction_option_idx,$data_no,$file_1_idx2="",$file_1_title2){
	...
	$avg_point = zero_point_set($row_data_2['analysis_report_satisfaction_100data_quiz_val'],$satisfaction_option_scorepoint);
	return $avg_point;
}

function get_satisfaction_100data_no_val_16($satisfaction_option_idx,$data_no,$file_1_idx2="",$file_2_idx2="",$file_1_title2=""){
	$satisfaction_option_scorepoint = "16";
	...
	if($file_2_idx2){
		$sql_data_2 .= " and analysis_report_satisfaction_100data_model_no='".$file_2_idx2."'"; 
	}
	if($file_1_title2){
		$sql_data_2 .= " and analysis_report_satisfaction_100data_quiz_title='".$file_1_title2."'"; 
	}
	...
	$avg_point = zero_point_set_16($row_data_2['analysis_report_satisfaction_100data_quiz_val'],$satisfaction_option_scorepoint);
	return $avg_point;
}
...
function get_satisf_100data_satisfval_all($satisfaction_option_idx,$quiz_no_where){
	...
	$opt_sql_cnt = "select analysis_report_satisfaction_100data_quiz_val from wise_analysis_report_satisfaction_100data_detail where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_quiz_no = '".$quiz_no_where."'";
	//echo "get_satisf_100data_satisfval opt_sql = ".$opt_sql_cnt."<br>"; //exit;
	$opt_query_cnt = mysqli_query($GLOBALS['gconnet'],$opt_sql_cnt);
	$opt_cnt_cnt = mysqli_num_rows($opt_query_cnt);
	//echo "get_satisf_100data_satisfval opt_cnt = ".$opt_cnt."<br>";

	//echo "데이터 수 ?? = ".$opt_cnt_cnt."<br>";
	
	if($opt_cnt_cnt == 0){
		$avg_point = 0;
	} else {
		$total_point = 0;
		for($opt_i2=0; $opt_i2<$opt_cnt_cnt; $opt_i2++){ // 각 변수별 문항루프 시작 
			$opt_row = mysqli_fetch_array($opt_query_cnt);
			$total_point = $total_point + $opt_row['analysis_report_satisfaction_100data_quiz_val'];
			//echo "개별 데이터 = ".$row_data_2['data_val']."<br>";
			//echo "합계 데이터 = ".$total_point."<br>";
		} // 각 변수별 문항루프 종료 
		//echo "총점 ?? = ".$total_point."<br>";
		$avg_point = zero_point_set($total_point/$opt_cnt_cnt,$satisfaction_option_scorepoint);
		//echo "평균 ?? = ".$opt_cnt_cnt."<br>";
	}
...
function get_service_100data_no_val($service_option_idx,$data_no,$file_1_idx2="",$file_1_group2="",$file_1_model2="",$file_1_title2){
	...
	} elseif($file_1_model2){
		$sql_data_2 .= " and analysis_report_service_100data_model_no='".$file_1_model2."'"; 
	} elseif($file_1_idx2){
	...
	$rt_val = zero_point_set_16($row_data_2['analysis_report_service_100data_quiz_val'],"16");
	return $rt_val;
}
...
function get_service_100data_tlc_val($service_option_idx,$data_no,$col_name){
	...
	if($col_name == "analysis_report_service_option_level_title" || $col_name == "analysis_report_service_100data_count"){
		$rt_val = $row_data_2['colval'];
	} else {
		$rt_val = zero_point_set_16($row_data_2['colval'],"16");
	}
	return $rt_val;
}
...
function get_ipa_x_val($satisfaction_option_idx,$analysis_report_idx){
	/*$sql_prev_1 = "select idx,(select quiz_no from wise_analysis_quiz where 1 and idx=wise_analysis_report_satisfaction_option_model_quiz.analysis_report_satisfaction_option_factor_no) as quiz_no from wise_analysis_report_satisfaction_option_model_quiz where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and satisfaction_option_model_idx in (select idx from wise_analysis_report_satisfaction_option_model where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_option_variable_type='X') order by idx asc"; // 독립변수 
	echo "x_data_base = ".$sql_prev_1."<br>";
	...
	for($opt_prev_1=0; $opt_prev_1<$query_prev_1_cnt; $opt_prev_1++){
		...
		if($opt_prev_1 == $query_prev_1_cnt-1){
			$search_prev_1 .= trim($row_prev_1['quiz_no']);
		} else {
			$search_prev_1 .= trim($row_prev_1['quiz_no']).",";	
		}
	}
		
	$detail_tb_name = "wise_analysis_report_satisfaction_100data_detail";

	$data_ipa_x_val = array();
	
		$sql_file_1 = "select distinct(analysis_report_satisfaction_100data_no) from ".$detail_tb_name." where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' order by CONVERT(analysis_report_satisfaction_100data_no, UNSIGNED) asc";
		echo "x_sql_file_1 = ".$sql_file_1."<br>";
		$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
		$query_file_1_cnt = mysqli_num_rows($query_file_1);
		...
		for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){
			...
			$opt_sql = "select analysis_report_satisfaction_100data_quiz_no,analysis_report_satisfaction_100data_quiz_title,analysis_report_satisfaction_100data_quiz_val from ".$detail_tb_name." where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_quiz_no in (".$search_prev_1.") and analysis_report_satisfaction_100data_no='".$row_file_1['analysis_report_satisfaction_100data_no']."' and analysis_report_satisfaction_100data_quiz_val != '' order by analysis_report_satisfaction_100data_quiz_no asc";
			
			echo "x val opt_sql = ".$opt_sql."<br>";
			$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
			$opt_cnt = mysqli_num_rows($opt_query);
			//echo $opt_cnt."<br>";
...
function get_ipa_y_val($satisfaction_option_idx,$analysis_report_idx){
	...
	$data_ipa_y_val = array();
		...
		$sql_file_1 = "select distinct(analysis_report_satisfaction_100data_no) from ".$detail_tb_name." where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' order by CONVERT(analysis_report_satisfaction_100data_no, UNSIGNED) asc";
		echo "y_sql_file_1 = ".$sql_file_1."<br>";
		$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
		$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	//echo "x_val = ".$ipa_y_val."<br><br>";
	//echo "x cnt = ".$query_file_1_cnt."<br>"; */

	$sql_prev_1 = "select idx,analysis_report_satisfaction_option_variable_title from wise_analysis_report_satisfaction_option_model where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_option_variable_type='M' order by idx asc"; // 중간변수
	//echo "sql_prev_1 = ".$sql_prev_1."<br>";
	$query_prev_1 = mysqli_query($GLOBALS['gconnet'],$sql_prev_1);
	$query_prev_1_cnt = mysqli_num_rows($query_prev_1);
	$search_prev_1 = "";
...
function set_rst_service_tlc($service_option_idx,$service_option_100data_idx,$service_100data_no,$analysis_report_idx){
	...
	$compet_info_query = mysqli_query($GLOBALS['gconnet'],$compet_info_sql);
	$row = mysqli_fetch_array($compet_info_query);
	$report_service_option_score = $row['analysis_report_service_option_scorepoint'];
	...
	//exit;
	$tot_score = zero_point_set($tot_score,$report_service_option_score);*/

	$sql_file_1 = "select analysis_report_service_100data_quiz_val from wise_analysis_report_service_100data_detail where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_no='".$service_100data_no."' and analysis_report_service_100data_model_no is not null and analysis_report_service_100data_model_no != ''";
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ // 응답자의 참여데이터 루프 시작 
		$row_file_1 = mysqli_fetch_array($query_file_1);
		$tot_score = $tot_score+$row_file_1['analysis_report_service_100data_quiz_val'];
	} // 응답자의 참여데이터 루프 종료 

	$tot_score = $tot_score/$query_file_1_cnt;
	//$tot_score = zero_point_set($tot_score,$report_service_option_score);
	$tot_score = $tot_score;
	//echo "총점 = ".$tot_score."<br>";
	//exit;

	$tot_score2 = zero_point_set($tot_score,$report_service_option_score);

	//$sql_file_3 = "select idx,analysis_report_service_option_levelyn_title from wise_analysis_report_service_option_levelyn where 1 and service_option_idx='".$service_option_idx."' and CONVERT(analysis_report_service_option_levelyn_spoint, UNSIGNED) <= '".$tot_score."' and CONVERT(analysis_report_service_option_levelyn_epoint, UNSIGNED) >= '".zero_point_set($tot_score,$report_service_option_score)."'"; // 등급기준

	$sql_file_3 = "select idx,analysis_report_service_option_levelyn_title from wise_analysis_report_service_option_levelyn where 1 and service_option_idx='".$service_option_idx."' and CONVERT(analysis_report_service_option_levelyn_spoint, UNSIGNED) <= '".$tot_score2."' and CONVERT(analysis_report_service_option_levelyn_epoint, UNSIGNED) >= '".$tot_score2."' order by idx asc limit 0,1";
	...
	if(mysqli_num_rows($query_file_3) == 0){
		...
		$query_in .= " analysis_report_service_100data_total2 = '".$tot_score2."', ";
		$query_in .= " wdate = now() ";
	} else {
		$row_file_3 = mysqli_fetch_array($query_file_3);
		$query_in = "insert into wise_analysis_report_service_100data_tlc set"; 
...
function set_rst_service_tlc_align($service_option_idx,$service_option_100data_idx){
	$sql_file_1 = "select idx,analysis_report_service_100data_total2 from wise_analysis_report_service_100data_tlc where 1 and service_option_idx='".$service_option_idx."' order by CAST(analysis_report_service_100data_total2 AS DECIMAL(4, 2)) desc";
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ // 총점 데이터 루프 시작 
		...
		$sql_file_2 = "select analysis_report_service_100data_count from wise_analysis_report_service_100data_tlc where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_total2='".$row_file_1['analysis_report_service_100data_total2']."' and analysis_report_service_100data_count > 0 order by analysis_report_service_100data_count asc limit 0,1";
		if($_SERVER['REMOTE_ADDR'] == "59.5.188.44"){
			echo $sql_file_2."<br>";
		}
		$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
		$query_file_2_cnt = mysqli_num_rows($query_file_2);
		$query_file_2_row = mysqli_fetch_array($query_file_2);

		$query_in = "update wise_analysis_report_service_100data_tlc set";
		if($query_file_2_cnt > 0){
			$query_in .= " analysis_report_service_100data_count = '".$query_file_2_row['analysis_report_service_100data_count']."'";
		} else {
...
function get_ipa_x_val_service($service_option_idx){
	...
	$data_ipa_x_val = array();
		...
		for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){
			...
			$opt_sql = "select analysis_report_service_100data_group_no,analysis_report_service_100data_quiz_title,analysis_report_service_100data_quiz_val from ".$detail_tb_name." where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_quiz_no in (".$search_prev_1.") and analysis_report_service_100data_no='".$row_file_1['analysis_report_service_100data_no']."' and analysis_report_service_100data_quiz_val != '' order by analysis_report_service_100data_quiz_no asc";
	...
	$sql_prev_1 = "select idx,analysis_report_service_option_samescyn_title from wise_analysis_report_service_option_samescyn where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_option_samescyn_jongs != 'Y' order by idx asc"; // 종속 아닌 분석모델
	//echo "sql_prev_1 = ".$sql_prev_1."<br>";
	$query_prev_1 = mysqli_query($GLOBALS['gconnet'],$sql_prev_1);
	$query_prev_1_cnt = mysqli_num_rows($query_prev_1);
	$search_prev_1 = "";
	...
	if(!$search_prev_1){ // 독립변수 없음 
		...
		$sql_file_1 = "select distinct(analysis_report_service_100data_no) from wise_analysis_report_service_100data_detail where 1 and service_option_idx='".$service_option_idx."' order by CONVERT(analysis_report_service_100data_no, UNSIGNED) asc";
		$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
		$query_file_1_cnt = mysqli_num_rows($query_file_1);
		...
		for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){
			...
			$opt_sql = "select analysis_report_service_100data_quiz_no,analysis_report_service_100data_quiz_title,analysis_report_service_100data_quiz_val from wise_analysis_report_service_100data_detail where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_model_no in (".$search_prev_1.") and analysis_report_service_100data_no='".$row_file_1['analysis_report_service_100data_no']."' order by analysis_report_service_100data_model_no asc";
...
function get_ipa_y_val_service($service_option_idx){
	...
	$sql_prev_1 = "select idx,analysis_report_service_option_samescyn_title from wise_analysis_report_service_option_samescyn where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_option_samescyn_jongs = 'Y' order by idx asc"; // 종속 분석모델
	//echo "sql_prev_1 = ".$sql_prev_1."<br>";
	$query_prev_1 = mysqli_query($GLOBALS['gconnet'],$sql_prev_1);
	$query_prev_1_cnt = mysqli_num_rows($query_prev_1);
	$search_prev_1 = "";
...
function get_perq_ipa_avg_service($service_option_idx,$model_no="",$quiz_title=""){
	...
	if($model_no){
		$opt_sql .= " and analysis_report_service_100data_model_no='".$model_no."'";
	}
	...
	$avg_point = zero_point_set($total_point/$opt_cnt,$report_service_option_score);
...
function get_service_data_val_avg($service_option_idx,$analysis_report_idx,$quiz_no,$tquiz_no,$tanswer){
	...
	//echo "get_service_data_val_avg sql = ".$opt_sql."<br>";
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
	//echo "get_service_data_val_avg cnt = ".$opt_cnt."<br>";
	if($opt_cnt == 0){
		$avg_point = 0;
	} else {
		$total_point = 0;
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){  
			...
			//echo "get_service_data_val_avg per = ".$service_100data_quiz_val."<br>";
			//echo "per = ".$service_100data_quiz_val."<br>";
			//echo "total = ".$total_point."<br>";
		}
		
		//echo "get_service_data_val_avg total = ".$total_point."<br>";
		$avg_point = zero_point_set($total_point/$opt_cnt,$service_option_scorepoint);
		//echo "get_service_data_val_avg avg = ".$avg_point."<br><br>";
	
	}
	return $avg_point;
}

function get_service_data_val_avg_tot($service_option_idx,$analysis_report_idx,$quiz_no,$tquiz_no,$tanswer){
	...
	$detail_tb_name = "wise_analysis_report_service_100data_tlc";
	
	$opt_sql .= "select analysis_report_service_100data_total from ".$detail_tb_name." where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_no in (select data_no from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."' and quiz_no='".$tquiz_no."' and data_val='".$tanswer."')";

	echo "get_service_data_val_tot sql = ".$opt_sql."<br>";
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
	echo "get_service_data_val_tot cnt = ".$opt_cnt."<br>";
	if($opt_cnt == 0){
		$avg_point = 0;
	} else {
		$total_point = 0;
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){  
			...
			$service_100data_quiz_val = $opt_row['analysis_report_service_100data_total'];		
			$total_point = $total_point + $service_100data_quiz_val;
			echo "get_service_data_val_tot per = ".$service_100data_quiz_val."<br>";
			//echo "per = ".$service_100data_quiz_val."<br>";
			//echo "total = ".$total_point."<br>";
		}
		
		echo "get_service_data_val_tot total = ".$total_point."<br>";
		$avg_point = zero_point_set($total_point/$opt_cnt,$service_option_scorepoint);
		echo "get_service_data_val_tot avg = ".$avg_point."<br><br>";
...
function get_service_data_val_avg2($service_option_idx,$analysis_report_idx,$quiz_no){
	...
	if($opt_cnt == 0){
		$avg_point = 0;
		$avg_point = zero_point_set($avg_point,$service_option_scorepoint);
	} else {
		$total_point = 0;
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){  
			$opt_row = mysqli_fetch_array($opt_query);
...
function get_satisf_loyalty_val($satisfaction_option_idx,$analysis_report_idx,$data_quiz_no="",$data_quiz_ans="",$loyalty){
	...
	$data_quiz_ans = get_calcurate_point($satisfaction_option_idx,$data_quiz_ans);
	//echo "데이터 응답값 = ".$data_quiz_ans."<br>";

	if($data_quiz_no && isset($data_quiz_ans)){ // 문항별 시작  
		...
		if($_SERVER['REMOTE_ADDR'] == "59.5.188.44" || $_SERVER['REMOTE_ADDR'] == "211.227.88.137"){
			//echo "문항별 sql_file_1 = ".$sql_file_1."<br>";
		}
		$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
		$query_file_1_cnt = mysqli_num_rows($query_file_1);
		...
		for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ // 문항조건 루프시작  
			...
			$sql_file_2 = "select idx from wise_analysis_report_satisfaction_100data_detail where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_no='".$row_file_1['analysis_report_satisfaction_100data_no']."' and analysis_report_satisfaction_100data_quiz_val='".$loyalty."'"; 
			
			if($_SERVER['REMOTE_ADDR'] == "59.5.188.44"){
				//echo "sql_file_2 = ".$sql_file_2."<br><br>";
			}
		...
		$sql_file_2 = "select idx from wise_analysis_report_satisfaction_100data_detail where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_quiz_val='".$loyalty."' and analysis_report_satisfaction_100data_no in (select analysis_report_satisfaction_100data_no from wise_analysis_report_satisfaction_100data_detail where 1 and analysis_report_satisfaction_100data_quiz_no in (select quiz_no from wise_analysis_quiz where 1 and analysis_idx='".$analysis_report_idx."'))"; 

		if($_SERVER['REMOTE_ADDR'] == "59.5.188.44" || $_SERVER['REMOTE_ADDR'] == "211.227.88.137"){
			//echo "전체값 sql_file_2 = ".$sql_file_2."<br>";
		}
...
function get_report_service_data_quiz_tot($service_option_idx,$service_data_idx,$where=""){
	
	$opt_sql = "select analysis_report_service_data_quiz_tot from wise_analysis_report_service_data_quiz where service_option_idx='".$service_option_idx."' and  service_data_idx='".$service_data_idx."'";
	if($where){
		$opt_sql .= $where;		
	}
	...
	return $opt_row['analysis_report_service_data_quiz_tot'];
}

function get_report_service_data_quiz_align($service_option_idx,$service_data_idx,$where=""){
	
	$opt_sql = "select analysis_report_service_data_quiz_align from wise_analysis_report_service_data_quiz where service_option_idx='".$service_option_idx."' and  service_data_idx='".$service_data_idx."'";
	if($where){
		$opt_sql .= $where;		
	}
	...
	return $opt_row['analysis_report_service_data_quiz_align'];
}
...
function set_report_service_data_quiz_val($service_option_idx,$file_1_quizno2,$title,$val){
	...
	if(mysqli_num_rows($opt_query) == 0){
		...
		$sql_file_1 = "select idx,analysis_report_service_data_quiz_val from wise_analysis_report_service_data_quiz where 1 and service_option_idx = '".$service_option_idx."' and analysis_report_service_data_quiz_no = '".$file_1_quizno2."' and service_data_idx is null order by analysis_report_service_data_quiz_val desc";
		$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
		$query_file_1_cnt = mysqli_num_rows($query_file_1);
		...
		for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ 
			...
			$sql_file_2 = "select analysis_report_service_data_quiz_align from wise_analysis_report_service_data_quiz where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_data_quiz_val='".$row_file_1['analysis_report_service_data_quiz_val']."' and analysis_report_service_data_quiz_val > 0 order by analysis_report_service_data_quiz_val asc limit 0,1";
			$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
			$query_file_2_cnt = mysqli_num_rows($query_file_2);
			$query_file_2_row = mysqli_fetch_array($query_file_2);
			...
			if($query_file_2_cnt > 0){
				$query_in .= " analysis_report_service_data_quiz_align = '".$query_file_2_row['analysis_report_service_data_quiz_align']."'";
			} else {
...
function set_report_service_data_total_align($service_option_idx,$analysis_report_idx){
	...
	for($opt4_i=0; $opt4_i<$query_file_4_cnt; $opt4_i++){ // 집단구분 보기 시작 
		...
		$row_tot_avg = zero_point_set($data_tot_1/sizeof($file_1_title_arr),$service_option_scorepoint);	
	...
	$sql_file_1 = "select distinct(service_data_idx) as data_idx from wise_analysis_report_service_data_quiz where 1 and service_data_idx is not null and service_option_idx='".$service_option_idx."'";  
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){
		...
		$sql_file_2 = "select idx,analysis_report_service_data_quiz_tot from wise_analysis_report_service_data_quiz where 1 and service_option_idx='".$service_option_idx."' and service_data_idx='".$row_file_1['data_idx']."' order by CAST(analysis_report_service_data_quiz_tot AS DECIMAL(4, 2)) desc";  
		$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
		$query_file_2_cnt = mysqli_num_rows($query_file_2);
		...
		for($opt_i2=0; $opt_i2<$query_file_2_cnt; $opt_i2++){
			...
			$sql_file_3 = "select analysis_report_service_data_quiz_align from wise_analysis_report_service_data_quiz where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_data_quiz_tot='".$row_file_1['analysis_report_service_data_quiz_tot']."' and analysis_report_service_data_quiz_align > 0 order by CAST(analysis_report_service_data_quiz_tot AS DECIMAL(4, 2)) asc asc limit 0,1";
			if($_SERVER['REMOTE_ADDR'] == "59.5.188.44"){
				echo $sql_file_3."<br>";
			}
			$query_file_3 = mysqli_query($GLOBALS['gconnet'],$sql_file_3);
			$query_file_3_cnt = mysqli_num_rows($query_file_3);
			$query_file_3_row = mysqli_fetch_array($query_file_3);

			$query_in = "update wise_analysis_report_service_data_quiz set";
			if($query_file_3_cnt > 0){
				$query_in .= " analysis_report_service_data_quiz_align = '".$query_file_3_row['analysis_report_service_data_quiz_align']."'";
			} else {
				$query_in .= " analysis_report_service_data_quiz_align = '".($opt_i2+1)."' ";
			}
			$query_in .= " where 1 and idx='".$row_file_2['idx']."'";
			//echo $query_in."<br>";
			$result_in = mysqli_query($GLOBALS['gconnet'],$query_in);
...
function get_ratio_val_service($service_option_idx,$analysis_report_idx,$quiz_no,$ans_no,$level_idx=""){
	...
	if($opt_tot_cnt == 0){
		...
		//$avg_point = zero_point_set($opt_cnt/$opt_tot_cnt,$service_option_scorepoint);
		$avg_point = $opt_cnt/$opt_tot_cnt;
...
function get_calcurate_point_damyun($damyun_option_idx,$orgscore){
	...
	echo "damyun_option_score = ".$damyun_option_score."<br>";
	echo "damyun_option_percent = ".$damyun_option_percent."<br>";
	echo "damyun_option_scorepoint = ".$damyun_option_scorepoint."<br>";
	echo "damyun_option_scorepoint2 = ".$damyun_option_scorepoint2."<br>";*/

	if(!$orgscore){
		$calcurate_point = 0;
		//echo "최종 calcurate_point = ".$calcurate_point."<br><br>";
	} else {
...
function get_calcurate_point_damyun_samescyn_quiz($damyun_option_idx,$analysis_report_idx,$quizno,$orgscore){
	...
	if($query_file_2_cnt == 0){
		...
		$row_file_2 = mysqli_fetch_array($query_file_2);
		if($row_file_2['analysis_report_damyun_option_samescyn_quiz_score']){ // 배점 설정되어 있을때 
			$bdscore = $row_file_2['analysis_report_damyun_option_samescyn_quiz_score']/100; // 배당 % 화 
			$caled_sc = $orgscore*$bdscore;

			########## 가중치 시작 ##########
				$quiz_idx2_sql = "select idx,(select analysis_report_damyun_option_weight_estimate_val1 from wise_analysis_report_damyun_option_point_weight_estimate where 1 and damyun_option_weight_idx=wise_analysis_report_damyun_option_point_weight.idx) as val_1,(select analysis_report_damyun_option_weight_estimate_val2 from wise_analysis_report_damyun_option_point_weight_estimate where 1 and damyun_option_weight_idx=wise_analysis_report_damyun_option_point_weight.idx) as val_2,(select analysis_report_damyun_option_weight_estimate_val3 from wise_analysis_report_damyun_option_point_weight_estimate where 1 and damyun_option_weight_idx=wise_analysis_report_damyun_option_point_weight.idx) as val_3 from wise_analysis_report_damyun_option_point_weight where 1 and damyun_option_idx='".$damyun_option_idx."' and analysis_report_damyun_option_weight_quiz_no='".$quiz_idx."'";
				$quiz_idx2_query = mysqli_query($GLOBALS['gconnet'],$quiz_idx2_sql);
				$quiz_idx2_row = mysqli_fetch_array($quiz_idx2_query);

				if($row_file_2['samescyn_type'] == "C1"){ // 상사
					if($quiz_idx2_row['val_1']){
						$caled_sc = $caled_sc*$quiz_idx2_row['val_1'];
					}
				} elseif($row_file_2['samescyn_type'] == "C2"){ // 부하
					if($quiz_idx2_row['val_1']){
						$caled_sc = $caled_sc*$quiz_idx2_row['val_3'];
					}
				} elseif($row_file_2['samescyn_type'] == "C3"){ // 동료
					if($quiz_idx2_row['val_1']){
						$caled_sc = $caled_sc*$quiz_idx2_row['val_2'];
					}
				}
			########## 가중치 종료 ##########
		} else { // 배점 설정 없을때
...
function get_calcurate_point_damyun_samescyn($damyun_option_idx,$damyun_100data_no,$damyun_option_samescyn_idx){
	...
	if($damyun_option_samescyn_idx){
		...
		echo "핵심구분 sql_file_2 = ".$sql_file_2."<br>"; //exit;
		$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
		$row_file_2 = mysqli_fetch_array($query_file_2);
		...
		echo "핵심구분 sql_file_3 = ".$sql_file_3."<br>";
		$query_file_3 = mysqli_query($GLOBALS['gconnet'],$sql_file_3);
		$row_file_3 = mysqli_fetch_array($query_file_3);
...
function set_rst_damyun_tlc($damyun_option_idx,$damyun_option_100data_idx,$damyun_100data_no,$analysis_report_idx){
	...
	//$tot_score = zero_point_set($tot_score,1);
...
function get_damyun_data_val_avg($damyun_option_idx,$analysis_report_idx,$quiz_no,$tquiz_no,$tanswer){
	...
	if($opt_cnt == 0){
		...
		$avg_point = zero_point_set($total_point/$opt_cnt,$damyun_option_scorepoint);
...
$compet_info_query = mysqli_query($GLOBALS['gconnet'],$compet_info_sql);
$row = mysqli_fetch_array($compet_info_query);
$report_damyun_option_score = $row['analysis_report_damyun_option_scorepoint'];
	...
	$avg_point = zero_point_set($total_point/$opt_cnt,$report_damyun_option_score);
	return $avg_point;
}
...
function set_report_damyun_data_total_align($damyun_option_idx,$analysis_report_idx){
	...
	for($opt4_i=0; $opt4_i<$query_file_4_cnt; $opt4_i++){ // 집단구분 보기 시작 
		...
		$row_tot_avg = zero_point_set($data_tot_1/sizeof($file_1_title_arr),$damyun_option_scorepoint);	
...
function set_rst_damyun_tlc_case($damyun_option_idx,$damyun_option_100data_idx,$damyun_100data_no,$analysis_report_idx){
	...
	//exit;
	$tot_score = zero_point_set($tot_score,$report_damyun_option_score);
...
function get_ratio_val_damyun($damyun_option_idx,$analysis_report_idx,$quiz_no,$ans_no,$level_idx=""){
	...
	if($opt_tot_cnt == 0){
		...
		$avg_point = zero_point_set($opt_cnt/$opt_tot_cnt,$damyun_option_scorepoint);
		$avg_point = $avg_point*100;
	}
...
function get_decision_100data_no_val($decision_option_idx,$data_no,$quiz_no,$file_1_idx2="",$file_1_group2="",$file_1_title2=""){
	...
	$sql_data_2 = "select analysis_report_decision_100data_quiz_val from wise_analysis_report_decision_100data_detail where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_no='".$data_no."'";
...
function get_decision_100data2_no_val($decision_option_idx,$data_no,$quiz_no,$file_1_idx2="",$file_1_group2="",$file_1_title2=""){
	$sql_data_2 = "select analysis_report_decision_100data_quiz_val from wise_analysis_report_decision_100data_detail2 where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_no='".$data_no."'";
...
function get_decision_100data3_no_val($decision_option_idx,$data_no,$quiz_no,$file_1_idx2="",$file_1_group2="",$file_1_title2=""){
	$sql_data_2 = "select analysis_report_decision_100data_quiz_val from wise_analysis_report_decision_100data_detail3 where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_no='".$data_no."'";
...
############# AHP 모델 계산 - 가중치법 시작 ###########
function calcurate_decision_100data_1($analysis_report_idx,$decision_100data_no,$title_sc,$opt_k,$quiz_title_chk_arr3){
	$quiz_title_chk_arr = explode(",",$quiz_title_chk_arr3);
	$opt_k_arr = $opt_k-1;
	//echo "opt_k = ".$opt_k."<br>";
	//echo "title_sc = ".$title_sc."<br>";
	
	if($opt_k == 1){ // 첫번째 문자열일때 
		$title_sc_tot_1 = 1;

		$sql_data_1 = "select quiz_no from wise_analysis_quiz where 1 and analysis_idx='".$analysis_report_idx."' and quiz_title like '".$title_sc."_%'";
		$query_data_1 = mysqli_query($GLOBALS['gconnet'],$sql_data_1);
	
		for($opt1_i=0; $opt1_i<mysqli_num_rows($query_data_1); $opt1_i++){  
			$row_data_1 = mysqli_fetch_array($query_data_1);
			$data_quiz_no = $row_data_1['quiz_no'];

			$sql_data_2 = "select data_val from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."' and quiz_no='".$data_quiz_no."' and data_no='".$decision_100data_no."'";
			$query_data_2 = mysqli_query($GLOBALS['gconnet'],$sql_data_2);
			$row_data_2 = mysqli_fetch_array($query_data_2);
			$title_sc_dan = $row_data_2['data_val']*(1/$row_data_2['data_val']);
			$title_sc_tot_1 = $title_sc_tot_1+$title_sc_dan;
		}
	} else { // 첫번째 이후 문자열일때 시작
		$opt_k_prev = $opt_k_arr-1;
		
		$prev_title_sc = $quiz_title_chk_arr[$opt_k_prev];
		$prev_title_ser = $prev_title_sc."_".$title_sc;
		
		$sql_data_1 = "select quiz_no from wise_analysis_quiz where 1 and analysis_idx='".$analysis_report_idx."' and quiz_title = '".$prev_title_ser."'";
		//echo "sql_data_1 = ".$sql_data_1."<br>";
		$query_data_1 = mysqli_query($GLOBALS['gconnet'],$sql_data_1);
		$row_data_1 = mysqli_fetch_array($query_data_1);
		$data_quiz_no_1 = $row_data_1['quiz_no'];

		$sql_data_1_1 = "select data_val from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."' and quiz_no='".$data_quiz_no_1."' and data_no='".$decision_100data_no."'";
		//echo "sql_data_1_1 = ".$sql_data_1_1."<br>";
		$query_data_1_1 = mysqli_query($GLOBALS['gconnet'],$sql_data_1_1);
		$row_data_1_1 = mysqli_fetch_array($query_data_1_1);
		$title_sc_dan_1 = 1*(1/$row_data_1_1['data_val']);
				
		$sql_data_2 = "select quiz_no from wise_analysis_quiz where 1 and analysis_idx='".$analysis_report_idx."' and quiz_title like '".$title_sc."_%'";
		//echo "sql_data_2 = ".$sql_data_2."<br>";
		$query_data_2 = mysqli_query($GLOBALS['gconnet'],$sql_data_2);
		
		$title_sc_dan_2_tot = 0;
		for($opt1_i=0; $opt1_i<mysqli_num_rows($query_data_2); $opt1_i++){  
			$row_data_2 = mysqli_fetch_array($query_data_2);
			$data_quiz_no = $row_data_2['quiz_no'];

			$sql_data_3 = "select data_val from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."' and quiz_no='".$data_quiz_no."' and data_no='".$decision_100data_no."'";
			//echo "sql_data_3 = ".$sql_data_3."<br>";
			$query_data_3 = mysqli_query($GLOBALS['gconnet'],$sql_data_3);
			$row_data_3 = mysqli_fetch_array($query_data_3);
			$title_sc_dan_2_1 = $row_data_3['data_val'];

			$opt_k_next = $opt_k_arr+($opt1_i+1);
			//echo "opt_k_next = ".$opt_k_next."<br>";
			$next_title_sc = $quiz_title_chk_arr[$opt_k_next];
			$next_title_ser = $prev_title_sc."_".$next_title_sc;
			//echo "opt_k_next_title = ".$next_title_ser."<br>";

			$sql_data_4 = "select quiz_no from wise_analysis_quiz where 1 and analysis_idx='".$analysis_report_idx."' and quiz_title = '".$next_title_ser."'";
			//echo "sql_data_4 = ".$sql_data_4."<br>";
			$query_data_4 = mysqli_query($GLOBALS['gconnet'],$sql_data_4);
			$row_data_4 = mysqli_fetch_array($query_data_4);
			$data_quiz_no_4 = $row_data_4['quiz_no'];

			$sql_data_2_2 = "select data_val from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."' and quiz_no='".$data_quiz_no_4."' and data_no='".$decision_100data_no."'";
			//echo "sql_data_2_2 = ".$sql_data_2_2."<br>";
			$query_data_2_2 = mysqli_query($GLOBALS['gconnet'],$sql_data_2_2);
			$row_data_2_2 = mysqli_fetch_array($query_data_2_2);
			$title_sc_dan_2_2 = 1/$row_data_2_2['data_val'];
			$title_sc_dan_2 = $title_sc_dan_2_1*$title_sc_dan_2_2;
			$title_sc_dan_2_tot = $title_sc_dan_2_tot+$title_sc_dan_2;
		}
		
		$title_sc_tot_1 = $title_sc_dan_1+$title_sc_dan_1+$title_sc_dan_2_tot;

	} // 첫번째 이후 문자열일때 종료 

	$title_sc_tot = $title_sc_tot_1;
	echo $title_sc." / ".$title_sc_tot."<br>";

	return $title_sc_tot;
}
############# AHP 모델 계산 - 가중치법 종료 ###########
/* 2022-03-11 : /inc/lib.php 이동 */
...
function get_option_url($rtype,$ridx){
	switch ($rtype) {
		case "manjok" : 
		$rtype_name = "/report_bn/manjok_1.php?satisfaction_option_idx=".$ridx."&chapter=new";
		break;
		case "service" : 
		$rtype_name = "/report_bn/service_1.php?service_option_idx=".$ridx."&chapter=new";
		break;
		case "damyun" : 
		$rtype_name = "/report_bn/damyun_1.php?damyun_option_idx=".$ridx."&chapter=new";
		break;
		case "decision" : 
		$rtype_name = "/report_bn/decision_1.php?decision_option_idx=".$ridx."&chapter=new";
		break;
	}
	return $rtype_name;
}

function zero_point_set($val,$zero_point){
	$val = round($val,$zero_point);
	...
	if($zero_point){
		//if(!$val_2){ // 소수점 없이 정수일때 시작 
		if(!isset($val_2)){
			$lzero = "";
			for($i=0; $i<$zero_point; $i++){
				$lzero .= "0"; 
			}
			$last_val = $val.".".$lzero;
		} else { // 소수점 있는 실수일때 시작 
			$val_2_size = strlen($val_2);
			if($val_2_size > $zero_point){
				$lzero = substr($val_2,0,$zero_point);
				$last_val = $val_1.".".$lzero;
			} else {
				$lzero = "";
				$minus_point = $zero_point-$val_2_size;
				for($i=0; $i<$minus_point; $i++){
					$lzero .= "0"; 
				}
				$last_val = $val_1.".".$val_2.$lzero;
			}
		} // 소수점 있는 실수일때 종료 
	} else {
		$last_val = $val;
...
function zero_point_set_16($val,$zero_point){
	if(!isset($val)){
		return "";
	}elseif($val == "4(우량충성집단)" || $val == "2(타성적충성집단)" || $val == "3(잠재적충성집단)" || $val == "1(비충성집단)"){
		return $val;
	} else {
		$val_arr = explode(".",$val);
		$val_1 = $val_arr[0];
...
function get_service_option_model_quiz_avg($service_option_100data_idx,$data_no,$service_option_idx,$option_model_idx){
	...
	$opt_sql = "select idx,analysis_report_service_option_samescyn_quiz_no,(select quiz_no from wise_analysis_quiz where 1 and idx=a.analysis_report_service_option_samescyn_quiz_no) as quiz_no,analysis_report_service_option_samescyn_quiz_score from wise_analysis_report_service_option_samescyn_quiz a where 1 and service_option_idx='".$service_option_idx."' and service_option_samescyn_idx='".$option_model_idx."' order by idx asc";
	//echo "opt_sql = ".$opt_sql."<br>";
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
	...
	if($opt_cnt == 0){
		...
		$sql_data_3 = "select sum(analysis_report_service_option_samescyn_quiz_score) as group_tot_val from wise_analysis_report_service_option_samescyn_quiz where 1 and service_option_idx='".$service_option_idx."' and service_option_samescyn_idx='".$option_model_idx."'";
		$query_data_3 = mysqli_query($GLOBALS['gconnet'],$sql_data_3);
		$row_data_3 = mysqli_fetch_array($query_data_3);
		$group_tot_val = $row_data_3['group_tot_val'];

		//echo "배점전체 = ".$group_tot_val."<br>";
		...
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){ // 각 변수별 문항루프 시작 
			...
			if($group_tot_val > 0){ // 배점있음 
				$a1_1 = $opt_row['analysis_report_service_option_samescyn_quiz_score'];
				$a1_2 = $a1_1/$group_tot_val; 
				//echo "①/(①+②) = ".$a1_2."<br>";
		
				$sql_data_1 = "select analysis_report_service_100data_quiz_val from wise_analysis_report_service_100data_detail where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_no='".$data_no."' and analysis_report_service_100data_quiz_no='".$quiz_no."'";
				$query_data_1 = mysqli_query($GLOBALS['gconnet'],$sql_data_1);
				$row_data_1 = mysqli_fetch_array($query_data_1);
				$a1 = $row_data_1['analysis_report_service_100data_quiz_val'];
				//echo "100data a1 = ".$a1."<br>";
				$a1 = $a1*$a1_2;
				//echo "100data a1*①/(①+②) = ".$a1."<br>";
			} else { // 배점없음 
				$sql_data_1 = "select analysis_report_service_100data_quiz_val from wise_analysis_report_service_100data_detail where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_no='".$data_no."' and analysis_report_service_100data_quiz_no='".$quiz_no."'";
			...
			$total_point = $total_point+$a1;
		} // 각 변수별 문항루프 종료 
		
		if($group_tot_val > 0){ // 배점있음 
		} else { // 배점없음
			$total_point = $total_point/$opt_cnt;
		} // 배점없음 종료 

		//echo "total_point = ".$total_point."<br><br>";
		//$avg_point = $total_point/$opt_cnt;
...
function get_satisf_data_group_detail_csi2($satisfaction_option_idx,$analysis_report_idx,$quiz_no,$quiz_val,$quiz_title=""){
	...
	if($opt_cnt == 0){
		...
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){ // 각 변수별 문항루프 시작 
			...
			$sql_data_2 = "select analysis_report_satisfaction_100data_quiz_val from wise_analysis_report_satisfaction_100data_detail where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_no='".$data_no."' and analysis_report_satisfaction_100data_quiz_title='CSI'"; 
			
			//echo "get_satisf_data_group_detail_avg2 sql_data_2 = ".$sql_data_2."<br>"; //exit;
			$query_data_2 = mysqli_query($GLOBALS['gconnet'],$sql_data_2);
			$row_data_2 = mysqli_fetch_array($query_data_2);
			$total_point = $total_point + $row_data_2['analysis_report_satisfaction_100data_quiz_val'];
		...
		//echo "get_satisf_data_group_detail_avg2 합계 데이터 = ".$total_point."<br>";
		...
		//echo "get_satisf_data_group_detail_avg2 평균값 = ".$avg_point."<br>"; 
	}
	//exit;
	return $avg_point;
}

function get_calcurate_decision_avg($data_100_model_no,$decision_option_idx,$file_1_title2,$type,$ilk_option_case_no=""){
	$memid = $_SESSION['wiz_session']['id'];
	$subid = $_SESSION['wiz_session']['subid'];

	$compet_info_sql = "select analysis_report_decision_option_scorepoint from wise_analysis_report_decision_option where 1 and idx='".$decision_option_idx."'";
	//echo $compet_info_sql."<br>";
	/*if($memid){
		$compet_info_sql .= " and memid='".$memid."'";
	}
	...
	if($data_100_model_no == "1"){
		$data_100_model_tb = "wise_analysis_report_decision_100data_detail";
	} elseif($data_100_model_no == "2"){
		$data_100_model_tb = "wise_analysis_report_decision_100data_detail2";
	} elseif($data_100_model_no == "3"){
		$data_100_model_tb = "wise_analysis_report_decision_100data_detail3";
	}

	echo "ilk_option_case_no = ".$ilk_option_case_no."<br>";

	if($ilk_option_case_no){
		$il_sql_file_2 = "select analysis_report_decision_100data_no from ".$data_100_model_tb." where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_ilk_yn = 'Y'";
		$il_query_file_2 = mysqli_query($GLOBALS['gconnet'],$il_sql_file_2);
		$query_file_1_cnt = mysqli_num_rows($il_query_file_2);
		for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ 
			$row_file_1 = mysqli_fetch_array($il_query_file_2);
			if($opt_i == $query_file_1_cnt-1){
				$file_1_data .= $row_file_1['analysis_report_decision_100data_no'];
			} else {
				$file_1_data .= $row_file_1['analysis_report_decision_100data_no'].",";
			}	
		}
	...
	$opt_sql = "select analysis_report_decision_100data_quiz_val from ".$data_100_model_tb." where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_quiz_title='".$file_1_title2."'";
	if($file_1_data){
		$opt_sql .= " and analysis_report_decision_100data_no in (".$file_1_data.")";
	}
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
	echo "get_calcurate_decision_avg opt_sql = ".$opt_sql."<br>";

	if($opt_cnt == 0){
		$avg_point = zero_point_set("0",$decision_option_scorepoint);
	} else {
		$total_point = array();
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){ // 각 변수별 문항루프 시작 
			$opt_row = mysqli_fetch_array($opt_query);
			$total_point[$opt_i2] = $opt_row['analysis_report_decision_100data_quiz_val'];
		} // 각 변수별 문항루프 종료 
		
		if($type == "avg"){ // 일반평균
			$avg_point = mean($total_point);
		} elseif($type == "avg2"){ // 기하평균
			$avg_point = geometricMean($total_point);
		}

		$avg_point = zero_point_set($avg_point,$decision_option_scorepoint);
	}
	
	return $avg_point;
}

function mean($a){ // 산술평균
  return array_sum($a) / count($a);
}

function geometricMean($a){ // 기하평균
  $mul = 0;
  foreach ($a as $i=>$v) {
    $mul = ($i === 0) ? $v : $mul * $v;
  }
...
function get_decision_100data_total_val($decision_option_idx,$model_no,$file_1_title2,$value_name){
	$sql_data_2 = "select ".$value_name." as data_val from wise_analysis_report_decision_100data_detail_total where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_model_no='".$model_no."'";
	...
	return $row_data_2['data_val'];
}

function get_calcurate_decision_excel_avg($data_100_model_no,$decision_option_idx,$file_1_title2,$type,$an_idx,$quiz_no,$answ,$ilk_option_case_no=""){
	$compet_info_sql = "select analysis_report_decision_option_scorepoint from wise_analysis_report_decision_option where 1 and idx='".$decision_option_idx."'";
	$compet_info_query = mysqli_query($GLOBALS['gconnet'],$compet_info_sql);
	$row = mysqli_fetch_array($compet_info_query);
	$decision_option_scorepoint = $row['analysis_report_decision_option_scorepoint'];
	...
	$opt_sql = "select analysis_report_decision_100data_quiz_val from ".$data_100_model_tb." where 1 and decision_option_idx='".$decision_option_idx."' and analysis_report_decision_100data_quiz_title='".$file_1_title2."' and analysis_report_decision_100data_no in (select data_no from wise_analysis_data where 1 and analysis_idx='".$an_idx."' and quiz_no='".$quiz_no."' and data_val='".$answ."')";
	if($file_1_data){
		$opt_sql .= " and analysis_report_decision_100data_no in (".$file_1_data.")";
	}
	//echo "get_calcurate_decision_excel_avg opt_sql = ".$opt_sql."<br>"; //exit;
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
	//echo "get_satisf_data_group_detail_avg2 opt_cnt = ".$opt_cnt."<br>";
