...
function get_calcurate_point($satisfaction_option_idx,$orgscore){
	...
	if($satisfaction_option_percent=="1"){ // 원점수 그대로 
		...
		//zero_point_set($calcurate_point,$satisfaction_option_scorepoint);
	} else { // 100 점 변환 
		$calcurate_point_1 = $satisfaction_option_score-1; // 만점 -1 

		/*echo "orgscore = ".$orgscore."<br>";
		echo "orgscore - 1 = ".($orgscore-1)."<br>";
		echo "satisfaction_option_score = ".$satisfaction_option_score."<br>";
		echo "calcurate_point_1 = ".$calcurate_point_1."<br>";*/
		...
		//$calcurate_point = zero_point_set($calcurate_point,$satisfaction_option_scorepoint);
		//$calcurate_point = zero_point_set($calcurate_point,$satisfaction_option_scorepoint);
	} // 구분 종료 
	
	//echo "calcurate_point = ".$calcurate_point."<br>";
	//$calcurate_point = "3.44445";
	//echo "round cal = ".zero_point_set($calcurate_point,$satisfaction_option_scorepoint)."<br>";
...
function set_rst_service_tlc_align($service_option_idx,$service_option_100data_idx){
	$sql_file_1 = "select idx,analysis_report_service_100data_total from wise_analysis_report_service_100data_tlc where 1 and service_option_idx='".$service_option_idx."' order by CONVERT(analysis_report_service_100data_total, UNSIGNED) desc";
	$query_file_1 = mysqli_query($GLOBALS['gconnet'],$sql_file_1);
	$query_file_1_cnt = mysqli_num_rows($query_file_1);
	...
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){ // 총점 데이터 루프 시작 
		...
		$sql_file_2 = "select analysis_report_service_100data_count from wise_analysis_report_service_100data_tlc where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_total='".$row_file_1['analysis_report_service_100data_total']."' and analysis_report_service_100data_count > 0 order by analysis_report_service_100data_count asc limit 0,1";
		//echo $sql_file_2."<br>";
		$query_file_2 = mysqli_query($GLOBALS['gconnet'],$sql_file_2);
		$query_file_2_cnt = mysqli_num_rows($query_file_2);
...
function get_calcurate_point_damyun($damyun_option_idx,$orgscore){
	...
	if(!$orgscore){
		...
		if($damyun_option_percent=="1"){ // 원점수 그대로 
			...
			echo "calcurate_point_1 = ".$calcurate_point_1."<br>";
			echo "orgscore - 1 = ".($orgscore-1)."<br>";
			echo "calcurate_point_2 = ".$calcurate_point_2."<br>";*/

			$calcurate_point = $calcurate_point_2*100;
			//echo "calcurate_point_2 * 100 = ".$calcurate_point."<br>";
			$calcurate_point = zero_point_set($calcurate_point,$damyun_option_scorepoint);
			//echo "최종 calcurate_point = ".$calcurate_point."<br><br>";
		} // 구분 종료 
	}
...
function get_service_option_model_quiz_avg($service_option_100data_idx,$data_no,$service_option_idx,$option_model_idx){
	...
	if($opt_cnt == 0){
		...
		/*$sql_data_2 = "select idx,(select quiz_no from wise_analysis_quiz where 1 and idx=a.analysis_report_service_option_samescyn_quiz_no) as quiz_no from wise_analysis_report_service_option_samescyn_quiz a where 1 and service_option_idx='".$service_option_idx."' and service_option_samescyn_idx='".$option_model_idx."' order by idx asc";
		$query_data_2 = mysqli_query($GLOBALS['gconnet'],$sql_data_2);
		
		$quiz_no_arr = "";
		for($opt_i2=0; $opt_i2<mysqli_num_rows($query_data_2); $opt_i2++){ // 해당 모델의 전체 문항번호 구하기 시작  
			$opt_row2 = mysqli_fetch_array($query_data_2);
			
			if($opt_i2 == (mysqli_num_rows($query_data_2)-1)){
				$quiz_no_arr .= $opt_row2['quiz_no'];
			} else {
				$quiz_no_arr .= $opt_row2['quiz_no'].",";
			}

		} // 해당 모델의 전체 문항번호 구하기 종료 

		$sql_data_3 = "select sum(analysis_report_service_100data_quiz_val) as group_tot_val from wise_analysis_report_service_100data_detail where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_no='".$data_no."' and analysis_report_service_100data_quiz_no in (".$quiz_no_arr.")";
		echo "배점전체 쿼리 = ".$sql_data_3."<br>";
		$query_data_3 = mysqli_query($GLOBALS['gconnet'],$sql_data_3);
		$row_data_3 = mysqli_fetch_array($query_data_3);
		$group_tot_val = $row_data_3['group_tot_val']; // 그룹 문항의 배점 전체 */
...
function get_satisf_data_group_detail_csi2($satisfaction_option_idx,$satisfaction_data_idx,$satisfaction_data_group_idx,$analysis_report_satisfaction_data_group2_no){
	...
	//echo "csi2 satisfaction_option_scorepoint = ".$satisfaction_option_scorepoint."<br>";

	$opt_sql = "select analysis_report_satisfaction_data_quiz_tot,analysis_report_satisfaction_data_quiz_val from wise_analysis_report_satisfaction_data_group_detail where 1 and satisfaction_option_idx = '".$satisfaction_option_idx."' and satisfaction_data_idx = '".$satisfaction_data_idx."' and satisfaction_data_group_idx = '".$satisfaction_data_group_idx."' and analysis_report_satisfaction_data_group2_no = '".$analysis_report_satisfaction_data_group2_no."' and analysis_report_satisfaction_data_quiz_no is not null";
	
	//$opt_sql = "select analysis_report_satisfaction_data_quiz_val,analysis_report_satisfaction_data_quiz_no from wise_analysis_report_satisfaction_data_group_detail where 1 and satisfaction_option_idx = '".$satisfaction_option_idx."' and satisfaction_data_idx = '".$satisfaction_data_idx."' and satisfaction_data_group_idx = '".$satisfaction_data_group_idx."' and analysis_report_satisfaction_data_group2_no = '".$analysis_report_satisfaction_data_group2_no."'";

	echo "get_satisf_data_group_detail_csi2 opt_sql = ".$opt_sql."<br>";
	
	$opt_query = mysqli_query($GLOBALS['gconnet'],$opt_sql);
	$opt_cnt = mysqli_num_rows($opt_query);
	echo "get_satisf_data_group_detail_csi2 opt_cnt = ".$opt_cnt."<br>";
	...
	if($opt_cnt == 0){
		...
		for($opt_i2=0; $opt_i2<$opt_cnt; $opt_i2++){ // 각 변수별 문항루프 시작 
			...
			//$a1 = $opt_row['analysis_report_satisfaction_data_quiz_tot'];
			$a1 = $opt_row['analysis_report_satisfaction_data_quiz_val'];
			$total_point = $total_point+$a1;

			/*$opt_sql_2 = "select analysis_report_satisfaction_100data_quiz_val from wise_analysis_report_satisfaction_100data_detail where 1 and satisfaction_option_idx = '".$satisfaction_option_idx."' and analysis_report_satisfaction_100data_quiz_no='".$opt_row['analysis_report_satisfaction_data_quiz_no']."'";
			echo "opt_sql_2 sql = ".$opt_sql_2."<br>";
			$opt_query2 = mysqli_query($GLOBALS['gconnet'],$opt_sql_2);
			$opt_row2 = mysqli_fetch_array($opt_query2);

			$a1 = $opt_row2['analysis_report_satisfaction_100data_quiz_val'];
			echo "opt_sql_2 a1 = ".$a1."<br>";
			$total_point = $total_point+$a1;*/
	...
	echo "get_satisf_data_group_detail_csi2 total_point = ".$total_point."<br>";
	echo "get_satisf_data_group_detail_csi2 avg_point = ".$avg_point."<br><br>";
	$avg_point = zero_point_set($avg_point,$satisfaction_option_scorepoint);
	return $avg_point;
