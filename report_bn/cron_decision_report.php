...
//$inc_report_sql = "select * from wise_analysis_myreport where 1 and report_status='tmp' and report_type='decision' and report_sdate is null order by dx desc limit 0,1";

$idx = trim(sqlfilter($_REQUEST['idx']));
echo $idx."<br>";
if($idx){
	/////if($idx == 201) {
		$sql = "UPDATE wise_analysis_myreport SET report_status='tmp' WHERE idx = '$idx'";
...
if($_SERVER['REMOTE_ADDR'] == "59.5.188.44"){
	$inc_report_sql = "select * from wise_analysis_myreport where 1 and report_type='decision' ".$top_where." order by idx asc limit 0,1";
} else {
	$inc_report_sql = "select * from wise_analysis_myreport where 1 and report_status='tmp' and report_type='decision' ".$top_where." order by idx asc limit 0,1";
}
...
for ($inc_report_i=0; $inc_report_i<mysqli_num_rows($inc_report_query); $inc_report_i++){ // 보고서 추출대상 루프 시작 
	...
	// 분석 공통테이블 추가 
	$inc_report_query_1 = "update analysis_report set stCode = 'PS' where idx='".trim($inc_report_row['idx'])."'"; 
	$inc_report_result_1 = mysqli_query($gconnet,$inc_report_query_1);
	...
	$ilk_option_case_no = ($ilk_option_case == "N") ? "" : trim($compet_info_row['analysis_report_decision_option_case_no']);
...
echo $decision_option_100data_idx."<br>";
	//exit;
	...
	echo "file_1_idx_arr =".$file_1_idx_arr."<br>";
	...
	$sql_data_model = "select distinct(decision_option_model_quiz_group) as quiz_group from wise_analysis_report_decision_option_model_quiz where 1 and decision_option_idx='".$decision_option_idx."'";
	$query_data_model = mysqli_query($gconnet,$sql_data_model);
	$model_max = mysqli_num_rows($query_data_model);
	...
	for($opt_i=0; $opt_i<$model_max; $opt_i++){ 
		$row_file_1 = mysqli_fetch_array($query_data_model);
		if($opt_i == $model_max-1){
			$quiz_parent .= $row_file_1['quiz_group'];
		} else {
			$quiz_parent .= $row_file_1['quiz_group'].",";
		}
	}
	...
	for($opt_i=0; $opt_i<$query_data_1_cnt; $opt_i++){ // 응답자 루프 시작 
		...
		$decision_100data_no = $row_data_1['data_no'];
			...
			for($opt2_i=0; $opt2_i<$model_max; $opt2_i++){ // 최고 레벨까지 루프 시작  
				...
				$decision_100data_quiz_no = 0;

				echo "opt2 = ".$quiz_parent_arr[$opt2_i]."<br>";
				echo "quizno2 => ".$decision_100data_quiz_no."<br>";
				...
				$sql_data_3 = "select *,(select quiz_no from wise_analysis_quiz where 1 and idx=wise_analysis_report_decision_option_model_quiz.analysis_report_decision_option_model_quiz_no) as quiz_no from wise_analysis_report_decision_option_model_quiz where 1 and decision_option_idx='".$decision_option_idx."' and decision_option_model_quiz_group='".$quiz_parent_arr[$opt2_i]."'";
				...
				for($opt_i3=0; $opt_i3<$query_data_3_cnt; $opt_i3++){ 
					...
					echo $opt_i3." = (".$query_data_3_cnt." - 1) <br>";
...
for($i=0;$i<$maxSizeValue;$i++){
	$sizeTemp = $i*($i+1)/2;
	//echo $i." => ".sizeof($inputNumber)." == ".$sizeTemp."<br>";
...
for($k=0;$k<1000;$k++){
	...
	for($i=0;$i<$sizeOfArray;$i++)$weightPre0[$i] = $weight[$i];	// 2022-05-25 : 소수점 아래로 차이나는 부분때문에 반올림 처리하지 않음 ----- kky
...
/*echo '고유치법 CI 최종 결과값 : '.$eigenValue;
echo "<br />";
echo "<br />";*/
...
echo '평균법 CI 최종 결과값 : '.$averageMethodValue;
echo "<br />";
echo "<br />";
...
echo '기하평균법 CI 최종 결과값 : '.$geometricMeanValue;
echo "<br />";
echo "<br />";
	...
	for($opt_i=0; $opt_i<sizeof($file_1_title_arr); $opt_i++){
		...
		echo "get_calcurate_decision_avg($data_100_model_no,$decision_option_idx,$file_1_title2,'avg',$ilk_option_case_no)";
	...
	// 분석 공통테이블 추가 
	$inc_report_query_2 = "update analysis_report set stCode = 'PE' where idx='".trim($inc_report_row['idx'])."'"; 
	$inc_report_result_2 = mysqli_query($gconnet,$inc_report_query_2);
