...
<?
	...
	$sub_sql_1 = "select * from wise_analysis_quiz where analysis_idx = '".$analysis_idx."' and quiz_delete='N' and answer_cnt = 1 order by quiz_title asc";
	//echo "2_3 = ".$sub_sql_1."<br>";
				...
				?>
                       <li id="ql_<?=$sub_row_1['idx']?>" class=" item_serv" >
                            <div class="node">
                                ...
                                <div class="score_div">
                                    <input type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" name="inc_left_quiz_score4_<?=$sub_row_1['idx']?>" id="inc_left_quiz_score4_<?=$sub_row_1['idx']?>" class="left_input_score4" onblur="set_calc_quiz_score4();">
                                </div>
                                <span class="jong_div">
                                    <input type="checkbox" name="inc_left_quiz_tscyn4_<?=$sub_row_1['idx']?>" id="inc_left_quiz_tscyn4_<?=$sub_row_1['idx']?>" value="Y" checked>
									<script>$("#inc_left_quiz_tscyn4_<?=$sub_row_1['idx']?>").trigger("click");</script>
                                </span>
                                <div class="dec_bwrap">
