...
<?
	...
	//echo "service_option_scorepoint = ".$service_option_scorepoint."<br>";
						...
						<table border>
							...
							?>
								<tr bgcolor=#ffffff align="center" height="22">
									...
									<td style="mso-number-format:'\@'"><?=number_format($row_file_1['analysis_report_service_data_frequency_val'])?></td>
									<td style="mso-number-format:'\@'"><?=$row_file_1['analysis_report_service_data_ratio_val']?></td>
								</tr>
							<?
							...
							?>
								...
								<?if($query_file_1_cnt > 0){?>
									...
									<td style="mso-number-format:'\@'"><?=$tot_ratio_val?></td>
								<?}else{
				...
				<tr>
					...
					<td>
						...
						<?} // 등급루프 종료?>
							...
							<tr bgcolor=#ffffff align="center" height="22">
								...
								?>
									<td style="mso-number-format:'\@'"><?=number_format($val_one)?></td>
								<?
									$val_tot = $val_tot+$val_one;
									} //등급루프 종료
								?>
								<td style="mso-number-format:'\@'"><?=number_format($val_tot)?></td>
							</tr>
					...
					?>
							<tr bgcolor=#ffffff align="center" height="22">
								...
								<td style="mso-number-format:'\@'"><?=iconv("UTF-8","EUC-KR",$row_file_4['data_val'])?></td>
								<?
								$val_tot = 0;
								for($opt2_i=0; $opt2_i<sizeof($file_2_idx_arr); $opt2_i++){ // 등급루프 시작 
									$file_2_idx2 = trim($file_2_idx_arr[$opt2_i]);
						...
						<?} // 등급루프 종료?>
							...
							<tr bgcolor=#ffffff align="center" height="22">
								...
								for($opt2_i=0; $opt2_i<sizeof($file_2_idx_arr); $opt2_i++){ // 등급루프 시작 
									...
									if($val_tot == 0){
										$val_per = 0;
										$val_per = zero_point_set($val_per,$service_option_scorepoint);
									} else {
										//$val_per = round($val_one/$val_tot,$service_option_scorepoint);
										$val_per1 = $val_one/$val_tot;
										$val_per1 = $val_per1*100;
										$val_per = zero_point_set($val_per1,$service_option_scorepoint);
										//$val_per = $val_per*100;
									}
								?>
									<td style="mso-number-format:'\@'"><?=$val_per?></td>
								<?
									} //등급루프 종료
								?>
								<td style="mso-number-format:'\@'"><?if($val_tot == 0){?>0<?}else{?><?=($val_tot/$val_tot)*100?><?}?></td>
							</tr>
