...
<?
	...
	$sql_file_1 = "select analysis_report_service_100data_no from wise_analysis_report_service_100data_detail where 1 and service_option_idx='".$service_option_idx."' order by analysis_report_service_100data_no asc limit 0,1"; 
	$query_file_1 = mysqli_query($gconnet,$sql_file_1);
	$row_file_1 = mysqli_fetch_array($query_file_1);
	
	$sql_file_1 = "select analysis_report_service_100data_quiz_no,analysis_report_service_100data_quiz_title,analysis_report_service_100data_group_no,analysis_report_service_100data_model_no from wise_analysis_report_service_100data_detail where 1 and service_option_idx='".$service_option_idx."' and analysis_report_service_100data_no='".$row_file_1['analysis_report_service_100data_no']."'"; // 문항 
	...
	for($opt_i=0; $opt_i<$query_file_1_cnt; $opt_i++){
		...
		$model_no = $row_file_1['analysis_report_service_100data_model_no'];
	...
	$file_1_model_arr = explode(",",$file_1_model);
	$file_1_title_arr = explode(",",$file_1_title);

	$sql_file_2 = "select analysis_report_service_100data_no from wise_analysis_report_service_100data_tlc where 1 and service_option_idx='".$service_option_idx."' order by analysis_report_service_100data_count asc";
		...
		?>
			<tr bgcolor=#ffffff align="center" height="22">
				...
				for($opt2_i=0; $opt2_i<sizeof($file_1_title_arr); $opt2_i++){
					...
					$file_1_model2 = trim($file_1_model_arr[$opt2_i]);
					$file_1_title2 = trim($file_1_title_arr[$opt2_i]);
				?>	
					<td style="mso-number-format:'\@'"><?=iconv("UTF-8","EUC-KR",get_service_100data_no_val($service_option_idx,$data_no,$file_1_idx2,$file_1_group2,$file_1_model2,$file_1_title2))?></td>
				<?}?>
					<td style="mso-number-format:'\@'"><?=iconv("UTF-8","EUC-KR",get_service_100data_tlc_val($service_option_idx,$data_no,"analysis_report_service_100data_total"))?></td>
					<td style="mso-number-format:'\@'"><?=iconv("UTF-8","EUC-KR",get_service_100data_tlc_val($service_option_idx,$data_no,"analysis_report_service_option_level_title"))?></td>
					<td style="mso-number-format:'\@'"><?=iconv("UTF-8","EUC-KR",get_service_100data_tlc_val($service_option_idx,$data_no,"analysis_report_service_100data_count"))?></td>
			</tr>
		<?}?>
		</table>
