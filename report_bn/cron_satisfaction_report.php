...
//$inc_report_sql = "select * from wise_analysis_myreport where 1 and report_status='tmp' and report_type='manjok' and report_sdate is null order by idx desc limit 0,1";
...
if($_SERVER['REMOTE_ADDR'] == "59.5.188.44"  || $_SERVER['REMOTE_ADDR'] == "211.227.88.137"){
	$inc_report_sql = "select * from wise_analysis_myreport where 1 and report_type='manjok' ".$top_where." order by idx desc limit 0,1";
} else {
	$inc_report_sql = "select * from wise_analysis_myreport where 1 and report_status='tmp' and report_type='manjok' ".$top_where." order by idx desc limit 0,1";
}

//$inc_report_sql = "select * from wise_analysis_myreport where 1 and report_type='manjok' order by idx desc limit 0,1";

//$inc_report_sql = "select * from wise_analysis_myreport where 1 and idx='17'";

echo "inc_report_sql = ".$inc_report_sql."<br>";
...
for ($inc_report_i=0; $inc_report_i<mysqli_num_rows($inc_report_query); $inc_report_i++){ // 보고서 추출대상 루프 시작 
	...
	$compet_info_sql = "select analysis_report_idx,analysis_report_satisfaction_option_scorepoint,analysis_report_satisfaction_option_scorepoint2,analysis_report_satisfaction_option_percent,(select quiz_no from wise_analysis_quiz where 1 and idx=a.analysis_report_satisfaction_option_loyer_stfno) as quiz_no_stf,(select quiz_no from wise_analysis_quiz where 1 and idx=a.analysis_report_satisfaction_option_loyer_lytno) as quiz_no_lyt from wise_analysis_report_satisfaction_option a where 1 and idx='".$satisfaction_option_idx."'";
	/*if($memid){
	...
	$query .= " report_idx = '".$inc_report_row['idx']."', ";
	$query .= " wdate = now() ";
	...
	//if($_SERVER['REMOTE_ADDR'] == "59.5.188.44"){
		$sql_file_3 = "select idx,(select quiz_no from wise_analysis_quiz where 1 and idx=wise_analysis_report_satisfaction_option_point_case.analysis_report_satisfaction_option_case_no) as quiz_no,analysis_report_satisfaction_option_case_answer as quiz_answ from wise_analysis_report_satisfaction_option_point_case where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' order by idx asc";
		...
		//echo "last case_data = ".$case_data."<br>";
		//exit;
				
	//}
	
	//echo "case_where = ".$case_where."<br>"; exit;
	...
	echo "sql_data_1 = ".$sql_data_1."<br>"; //exit;
	...
	for($opt_i=0; $opt_i<$query_data_1_cnt; $opt_i++){ // 응답자 루프 시작 
		...
		######## raw data 시작 ############
			...
			for($opt2_i=0; $opt2_i<mysqli_num_rows($query_data_2); $opt2_i++){ // 로우데이터 루프 시작 
				...
				//echo "satisfaction_100data_quiz_conf = ".$satisfaction_100data_quiz_conf."<br>";

				/*if(in_array($satisfaction_100data_quiz_no,$file_2_quizno_arr)){
					//echo $satisfaction_100data_quiz_no." / 포함 <br>";
					if(trim($satisfaction_100data_quiz_conf) == "Y"){
						$satisfaction_100data_quiz_val = zero_point_set($row_data_2['data_val'],$satisfaction_option_scorepoint);
					...
					//echo $satisfaction_100data_quiz_no." / 비포함 <br>";
					$satisfaction_100data_quiz_val = zero_point_set($row_data_2['data_val'],$satisfaction_option_scorepoint);
					//$satisfaction_100data_quiz_val = $row_data_2['data_val'];
				}*/
				...
				//echo "satisfaction_100data_quiz_val = ".$satisfaction_100data_quiz_val."<br>";
				...
				$query_in .= " wdate = now() ";
				if($satisfaction_100data_quiz_no == "42"){
	...
	################ 고객만족도 및 충성도 상/하 입력 시작 #################### 
	$sql_data_1 = "select distinct(data_no) from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."' ".$case_where." order by data_no asc";
	$query_data_1 = mysqli_query($gconnet,$sql_data_1);
	$query_data_1_cnt = mysqli_num_rows($query_data_1);

	if($quiz_no_stf){
		$s_100data_satisfval_all = get_satisf_100data_satisfval_all($satisfaction_option_idx,$quiz_no_stf); // 만족도 문항의 전체 평균값
	}
	if($quiz_no_lyt){
		$s_100data_loyalval_all = get_satisf_100data_satisfval_all($satisfaction_option_idx,$quiz_no_lyt); // 충성도 문항의 전체 평균값 
	}
	...
	for($opt_i=0; $opt_i<$query_data_1_cnt; $opt_i++){ // 응답자 루프 시작 
		...
		$satisfaction_100data_no = $row_data_1['data_no'];
			...
			if($quiz_no_stf){
				$s_100data_satisfval = get_satisf_100data_satisfval($satisfaction_option_idx,$satisfaction_100data_no,$quiz_no_stf); // 만족도 문항의 자기점수 
			} 
			
			if($quiz_no_lyt){
				$s_100data_loyalval = get_satisf_100data_satisfval($satisfaction_option_idx,$satisfaction_100data_no,$quiz_no_lyt); // 충성도 문항의 자기점수 
			} 

			/*echo $quiz_no_stf." 만족도 문항 전체 평균 = ".$s_100data_satisfval_all."<br>";
			echo $quiz_no_stf." 만족도 문항 자기점수 = ".$s_100data_satisfval."<br>";
			echo $quiz_no_lyt." 충성도 문항 전체 평균 = ".$s_100data_loyalval_all."<br>";
			echo $quiz_no_lyt." 충성도 문항 자기점수 = ".$s_100data_loyalval."<br><br>";*/

			//if($s_100data_satisfval >= $satisfaction_100data_csi_val){
			if($quiz_no_stf){
				if($s_100data_satisfval >= $s_100data_satisfval_all){
			...
			if($quiz_no_lyt){
				if($s_100data_loyalval >= $s_100data_loyalval_all){
	...
	for($opt_i=0; $opt_i<mysqli_num_rows($query_data_1); $opt_i++){ // 로우데이터 그룹별 루프 시작 
		...
		for($opt2_i=0; $opt2_i<mysqli_num_rows($query_data_2); $opt2_i++){ // 로우데이터 그룹별 세부문항 루프 시작 
			...
			//$analysis_report_satisfaction_data_group2_no = get_calcurate_point($satisfaction_option_idx,$row_data_2['data_val']);
			...
			if(!$case_where){
				...
				$sql_data_3 = "select * from wise_analysis_data where 1 and analysis_idx='".$analysis_report_idx."' and data_no='".$case_data_arr[0]."' and quiz_no in (select quiz_no from wise_analysis_quiz where 1 and analysis_idx='".$analysis_report_idx."' and (idx in (select analysis_report_satisfaction_option_group_no from wise_analysis_report_satisfaction_option_group where 1 and satisfaction_option_idx='".$satisfaction_option_idx."') or idx in (select analysis_report_satisfaction_option_factor_no from wise_analysis_report_satisfaction_option_model_quiz where 1 and satisfaction_option_idx='".$satisfaction_option_idx."' and  analysis_report_satisfaction_option_factor_type='P'))) order by CONVERT(quiz_no, UNSIGNED) asc";
			}

			//echo $sql_data_3."<br>"; exit;
			...
			for($opt3_i=0; $opt3_i<mysqli_num_rows($query_data_3); $opt3_i++){ // 로우데이터 루프 시작 
				...
				//$avg_val = zero_point_set($row_data_2['data_val'],$satisfaction_option_scorepoint);
				//$avg_val = $row_data_2['data_val'];
				
				$avg_val = get_calcurate_point($satisfaction_option_idx,$row_data_2['data_val']);
			    ...
			    $analysis_report_satisfaction_data_quiz_val = get_satisf_data_group_detail_avg($satisfaction_option_idx,$analysis_report_idx,$row_data_1['quiz_no'],$avg_val,$row_data_3['quiz_no']);

				//$analysis_report_satisfaction_data_quiz_tot = get_satisf_data_group_detail_tot($satisfaction_option_idx,$analysis_report_idx,$row_data_1['quiz_no'],$avg_val,$row_data_3['quiz_no']);
				...
				$query_in2 .= " analysis_report_satisfaction_data_quiz_tot = '".$analysis_report_satisfaction_data_quiz_tot."', ";
				$query_in2 .= " wdate = now() ";
			...
			for($opt3_i=0; $opt3_i<sizeof($file_1_idx_arr); $opt3_i++){ // 독립변수 루프 시작 
				...
				$csi_avg_val = get_calcurate_point($satisfaction_option_idx,$row_data_2['data_val']);

				$analysis_report_satisfaction_data_quiz_val = get_satisf_data_group_detail_avg2($satisfaction_option_idx,$analysis_report_idx,$row_data_1['quiz_no'],$csi_avg_val,$file_1_idx2,$file_1_title2);

				//$analysis_report_satisfaction_data_quiz_val = get_satisf_data_group_detail_avg2($satisfaction_option_idx,$analysis_report_idx,$row_data_1['quiz_no'],$row_data_2['data_val'],$file_1_idx2,$file_1_title2);
			...
			//$analysis_report_satisfaction_data_quiz_val = get_satisf_data_group_detail_avg2($satisfaction_option_idx,$analysis_report_idx,$row_data_1['quiz_no'],$avg_val,"CSI");
			...
			//$analysis_report_satisfaction_data_quiz_val = get_satisf_data_group_detail_csi2($satisfaction_option_idx,$satisfaction_data_idx,$satisfaction_data_group_idx,$analysis_report_satisfaction_data_group2_no);
			
			$csi_avg_val = get_calcurate_point($satisfaction_option_idx,$row_data_2['data_val']);

			$analysis_report_satisfaction_data_quiz_val = get_satisf_data_group_detail_csi2($satisfaction_option_idx,$analysis_report_idx,$row_data_1['quiz_no'],$csi_avg_val,"");
	...
	echo "data_ipa_x_val cnt = ".sizeof($data_ipa_x_val)."<br>"; 
	$data_ipa_y_val = get_ipa_y_val($satisfaction_option_idx,$analysis_report_idx);
	echo "data_ipa_y_val cnt = ".sizeof($data_ipa_y_val)."<br>";
	//exit;
	
	if($_SERVER['REMOTE_ADDR'] == "59.5.188.44"){
		//echo "line 625 <br>"; exit;
	}

	if(sizeof($data_ipa_x_val) > 0 && sizeof($data_ipa_y_val) > 0){
		include $_SERVER["DOCUMENT_ROOT"]."/report/pro_inc/include_ipa_data.php"; // IPA 데이터 추출 인클루드
	}
	//exit;
	
	if($_SERVER['REMOTE_ADDR'] == "59.5.188.44"){
		//echo "line 634 <br>"; exit;
	}
	...
	//$ipa_total_t = zero_point_set($ipa_total_t,14);

	echo "ipa_total_t = ".$ipa_total_t."<br>";

	if(trim($ipa_total_t) == "NAN"){
		echo "ipa_total_t nan <br>";
		$ipa_total_t = zero_point_set("0",$satisfaction_option_scorepoint);
	} else {
		echo "ipa_total_t nan2 <br>";
		$ipa_total_t = zero_point_set($ipa_total_t,$satisfaction_option_scorepoint);
	}
	...
	for($opt3_i=0; $opt3_i<sizeof($file_1_idx_arr); $opt3_i++){ // 독립변수 루프 시작 
		...
		if(trim($getTStats[$opt3_i]) == "NAN"){
			$analysis_report_satisfaction_statistics_ipa_t = zero_point_set("0",$satisfaction_option_scorepoint);
		} else {	
			$analysis_report_satisfaction_statistics_ipa_t = zero_point_set($getTStats[$opt3_i],$satisfaction_option_scorepoint);
		}
		...
		if(trim($getTStats[$opt3_i]) == "NAN"){
			$analysis_report_satisfaction_statistics_ipa_tabval = zero_point_set("0",$satisfaction_option_scorepoint);
		} else {	
			$analysis_report_satisfaction_statistics_ipa_tabval = zero_point_set($getTStats[$opt3_i],$satisfaction_option_scorepoint);
		}
		...
		if($ipa_total_t == zero_point_set("0",$satisfaction_option_scorepoint)){
			$analysis_report_satisfaction_statistics_ipa_ratio = zero_point_set("0",$satisfaction_option_scorepoint);
		} else {
			$analysis_report_satisfaction_statistics_ipa_ratio = zero_point_set($analysis_report_satisfaction_statistics_ipa_t/$ipa_total_t,$satisfaction_option_scorepoint);
		}
	...
	$ipa_avg_stf = zero_point_set($ipa_total_stf/sizeof($file_1_idx_arr),$satisfaction_option_scorepoint); 
	$ipa_avg_imt = zero_point_set($ipa_total_imt/sizeof($file_1_idx_arr),$satisfaction_option_scorepoint); 
